...
module.exports = {
   ...
   * @return {json}
   */
  auctionDetail: asyncWrap(
    async (req, res) => {
      let auction = await AuctionRepository.getDetailAuction(
  ...
  getAuctionChancesSold: asyncWrap(async (req, res) => {
    ...
    let chancesSold = await AuctionRepository.getAuctionChancesSold(
      auctionId,
      page
    );
  ...
  getAuctionHistory: asyncWrap(async (req, res) => {
    ...
    let auctionsHistory = await AuctionRepository.getAuctionHistory(
      product,
      page
    );
  ...
  buyChances: asyncWrap(async (req, res) => {
    ...
    let result = await AuctionRepository.buyChances(
      auctionsInCart,
      discountTickets,
      req.user._id
    );
  ...
  search: asyncWrap(async (req, res) => {
    ...
    if (!keyword) return res.badRequest(sails.errors.requireKeyword);
