...
module.exports = {
    ...
    getAllReviews: asyncWrap(async (req, res) => {
        ...
        let reviews = await Review.paginate(query, sails.helpers.optionPaginateAdmin(req, [userModel, auctionModel], page));
        res.ok({data: reviews});
    }),
    ...
    blockReview : asyncWrap( async (req, res) => {
        ...
        let review  = await Review.findByIdAndUpdate(id, {$set: {status : 0}}, {new: true});
        res.ok({data : review});
    }),
    ...
    approveReview : asyncWrap( async (req, res) => {
        let id = req.params.id;
        let review  = await Review.findByIdAndUpdate(id, {$set: {status : 1}}, {new: true});
        res.ok({data : review});
    }),
    ...
    deleteReview: asyncWrap(async (req, res) => {
        ...
        let review = await Review.findByIdAndUpdate({_id: id}, {$set: {deletedAt}}, {new: true});
        res.ok({data: review});
    })
};
