...
module.exports = {
  ...
  getProducts: asyncWrap(async (req, res) => {
    ...
    const product = await Product.paginate(
      query,
      sails.helpers.optionPaginateAdmin(req, [popCate, popCondition], page)
    );
    res.ok({ data: product });
  }),
  ...
  postCreate: asyncWrap(async (req, res) => {
    ...
    product.chanceNumber = Math.floor(
      req.body.price / sails.config.value1Chance
    );
  ...
  postUpdate: asyncWrap(async (req, res) => {
    ...
    let promise = await Promise.all([
      ...
      Product.findByIdAndUpdate(id, { $set: product }, { new: true }).populate([
        popCate,
        popCondition
      ])
    ]);
