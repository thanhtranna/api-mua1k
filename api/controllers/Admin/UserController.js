...
const PATH = {
  dirname: require('path').resolve(sails.config.appPath, '.tmp/uploads/origin')
};
...
module.exports = {
  ...
  putUserBlock: asyncWrap(async (req, res) => {
    ...
    if (userReport) {
      ...
      await Report.findByIdAndUpdate(
        userReport._id,
        { $set: { isHandle: !userReport.isHandle } },
        { new: true }
      );
    }
  ...
  postUpdateUser: asyncWrap(async (req, res) => {
    ...
    user.avatar = upload;
    let person = await User.findByIdAndUpdate(
      id,
      { $set: user },
      { new: true }
    );
  ...
  putUserCommentBlock: asyncWrap(async (req, res) => {
    let id = req.params.id;
    let comment = await UserComment.findByIdAndUpdate(
      id,
      { $set: { status: 0 } },
      { new: true }
    );
  ...
  getUserReports: asyncWrap(async (req, res) => {
    let page = req.query.page || 1;
    let reports = await Report.paginate(
      {},
      sails.helpers.optionPaginateAdmin(
        req,
        [popFromUserReport, popToUserReport],
        page
      )
    );
    res.ok({ data: reports });
