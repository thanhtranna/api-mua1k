...
module.exports = {
    ...
    fakeCategory: asyncWrap(async (req, res) => {
        ...
        for (let i = 0; i < quantity; i++) {
            let category = {
                ...
                icon: {
                    origin: 'https://loremflickr.com/640/480/transport',
                    thumb: 'https://loremflickr.com/640/480/transport'
                }
            };
    ...
    fakeProduct: asyncWrap(async (req, res) => {
        ...
        function randomImages() {
            ...
            for (let i = 0; i < MIN_IMAGES; i++) {
                let imageUrl = 'https://loremflickr.com/640/480/fashion';
                let image = {
                    thumb: imageUrl,
                    origin: imageUrl
                };
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            let product = {
                ...
                images: randomImages(),
                featureImage: {
                    thumb: 'https://loremflickr.com/640/480/fashion',
                    origin: 'https://loremflickr.com/640/480/fashion'
                },
                price: chance * 100,
                isFavorite: true,
	...
	fakeAuction: asyncWrap(async (req, res) => {
		...
		for (let i = 0; i < quantity; i++) {
			let product = products[faker.random.number(products.length - 1)],
				startAt = new Date(2018, 4, faker.random.number(30)),
				expiredAt = new Date(2018, 10, faker.random.number(30)),
                aid;
	...
	fakeUser: asyncWrap(async (req, res) => {
		...
		for (let i = 0; i < quantity; i++) {
			...
			await User.update({_id: user._id}, {addressId: address._id}, {new: true});
	...
	fakeAuctionWinner: asyncWrap(async (req, res) => {
		...
		for(let i=0; i< auctions.length; i++) {
			...
			if(auction.aid % 5 === 0) {
				...
				await Auction.update({_id:auction.id}, {$set: {status:3, luckyNumber:1000001+faker.random.number(parseInt(auction.chanceNumber))}});
				...
				if(chanceBuy > 0) {
                    ...
                    for(let i = 0; i< chanceBuy; i++) {
                        if(chance > 0) {
                            ...
                            while (luckyNumberArr.indexOf(luckyNumber) === -1 || luckyNumberArr.length === 0) {
    ...
    fakeUserChanceBuy2: asyncWrap(async (req, res) => {
        ...
        let users = await User.find({}),
            ...
            }).select('id chanceNumber firstBuyUser lastBuyUser mostBuyUser status luckyNumber');
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            let sumChanceSold = 0;
            let usersBuyThisAuction = await UserChanceBuy.find({ auction: auction.id });
            usersBuyThisAuction.forEach(document => sumChanceSold += document.number);

            //return if don't enough chance
            if (auction.chanceNumber - sumChanceSold <= 0) return res.badRequest({message: "don't enough chance"});
            //
            let number = (auction.chanceNumber - sumChanceSold <= 0) ? 1 : faker.random.number((auction.chanceNumber - sumChanceSold > 30) ? 30 : auction.chanceNumber - sumChanceSold);
            ...
            let luckyNumbers = await AuctionService.getRandomLuckyNumbers(auction.id, number);
            let logs = [];
            ...
            if ((auction.chanceNumber - sumChanceSold - createdChanceBuy.number) === 0){
                auction.lastBuy = createdChanceBuy.id;
                auction.finishAt = moment().add(1, 'h').format();
                auction.status = 2;
                await auction.save();
            }
        }
    ...
    fakeLogUserPoints: asyncWrap(async (req, res) => {
        ...
        for (let i = 0; i < quantity; i++) {
            let log = {
                user : users[faker.random.number(users.length - 1)]._id,
                point : faker.random.number(1000),
                task : tasks[faker.random.number(tasks.length) - 1]._id,
    ...
    fakeWinner: asyncWrap (async (req, res) => {
        let quantity = req.params.quantity;
        let auctions = await Auction.find({status: 2}).select('id status chanceNumber');
        let users = await User.find({isVerified: true});
        let winners =[];

        if (!auctions.length || !users.length) return res.badRequest({message: "no auctions have status:2 or users"});
    ...
    fakeLogUserChanceBuy : asyncWrap(async (req, res) => {
        ...
        for(let i in quantity) {
            let data = {
                ...
                userChanceBuy: userChanceBuys[faker.random.number(userChanceBuys.length -1)]._id,
                luckyNumber: faker.random.number(1000)
            }
            datas.push(data);
        }
        console.log('Data log user chance buy: ', datas);
    ...
    fakeMessage : asyncWrap(async (req, res) => {
        ...
        for (let i = 0; i < quantity; i++) {
            let msg = {
                ...
                category: messageCategory[faker.random.number(messageCategory.length -1)]._id
            };
            messages.push(msg);
        }
    ...
    fakeContacts: asyncWrap(async (req, res) => {
        ...
        for(let i = 0; i < quantity; i++) {
            const contact = {
                ...
                email: faker.internet.email(),
                content: faker.lorem.paragraph(),
                category: contactCategory[faker.random.number(contactCategory.length -1)]._id,
    ...
    calculateNumberB: async (req, res) => {
        try {
            let finishAt = moment().subtract(req.query.mins, 'm').format();
            ...
            let temperature = $(`${mainRow} td:nth-child(${temperatureColumn})`).html();
        ...
        for(let i in users) {
            ...
            for(let j=0; j<totalFriend; j++) {
                if(users[i] === users[j]) {
                    ...
                    switch (status) {
                        case 1: totalBestFriend++;
                            break;
                        case 2: totalGoodFriend++;
                            break;
                        case 3: totalNormalFriend++;
                            break;
                    }
                    let friend = {
                        user: users[j]._id,
    ...
    fakeUserDiscountTicket: asyncWrap(async (req, res) => {
        ...
        for(let i = 0; i < promise[0].length; i++) {
            ...
            let saveUserDiscountTicket = await UserDiscountTicket.create(userDiscountTicket);
